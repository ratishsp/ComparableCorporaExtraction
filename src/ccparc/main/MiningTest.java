    ...
    static {
      ...
      */
      ...
      languageIds.put("Bengali", Collections.unmodifiableList(
    		  Arrays.asList("bengali","bn","ben","bangla")));
      ...
      languageIds.put("Hindi", Collections.unmodifiableList(
    		  Arrays.asList("hindi","hi","hin")));
      languageIds.put("Marathi", Collections.unmodifiableList(
    		  Arrays.asList("marathi","mr","mar")));
      languageIds.put("Malayalam", Collections.unmodifiableList(
    		  Arrays.asList("malayalam","ml","mal")));
...
//    		  Arrays.asList("Indonesian","Bahasa Indonesia","id","ind")));
          ...
          while (m.find()) {
            ret.add (
              new LanguageIndependentUrl (
                ...
                strUrl,
                REVERSE_LANGUAGE_IDS.get (m.group().toLowerCase())
              )
            );
          }
        }
  ...
  public int run(String[] args)
    ...
    if (args.length <  1)
      throw new IllegalArgumentException("Example JAR must be passed output paths.");

//    inputPath = args[0];
    inputPath = "s3n://aws-publicdatasets/common-crawl/parse-output/segment/1346823846036/*.arc.gz";
    outputPath = args[0];
  ...
  /*public static void main(String[] args) {
	  List<LanguageIndependentUrl> urls = MiningTestMapper.getLanguageIndependentUrls("poetryinstone.in/lang/Tamil/2010/03/18/pallava-cave-temple-at-thiruchirappalli-rockfort-lalitankura-pallavesvara-griham.html");
